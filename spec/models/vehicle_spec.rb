...
RSpec.describe Vehicle, type: :model do
  describe '#valid?' do
    it 'falso quando a placa não é única' do
      ...
      vehicle = Vehicle.create!(license_plate: 'BCD2222', brand:'Toyota', model:'Caminhão', year: 2018, capacity:2000000, modality: modality)
      second_vehicle = Vehicle.create(license_plate: 'BCD2222', brand:'Ford', model:'Ka', year: 2015, capacity:10000, status:'maintenance')
    ...
    it 'falso quando o ano é inválido' do
      user = User.create(name: 'João', email: 'joao@email.com.br', password: 'password', role: 'admin')
      modality = Modality.create!(name: 'Caminhão', min_weight: 0, max_weight: 1000, min_distance: 51, max_distance:150, tax: 5)
      vehicle = Vehicle.create(license_plate: 'BCD2222', brand:'Toyota', model:'Caminhão', year: 1800, capacity:2000000, modality: modality)
    ...
    it 'falso quando a capacidade não é um número' do
      user = User.create(name: 'João', email: 'joao@email.com.br', password: 'password', role: 'admin')
      modality = Modality.create!(name: 'Caminhão', min_weight: 0, max_weight: 1000, min_distance: 51, max_distance:150, tax: 5)
      vehicle = Vehicle.create(license_plate: 'BCD2222', brand:'Toyota', model:'Caminhão', year: 2020, capacity:'lalala', modality: modality)
